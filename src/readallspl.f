     ...
     &     ,NOREG,WHOLEGRID,stfil)
      IMPLICIT NONE
c------------------------------------------
c
c Purpose:
c This routine reads a section of a wavepacket propagation into matrices,
...
d     write(*,*)'>>',fpr(1),fpr(2),fpr(3),fpr(4)
         ...
         IF(WHOLEGRID.EQ.1)THEN

            READ(10,*) wwork, wp, T(K), pol
            IF(NOREG.EQ.1) READ(10,*) wwork, wp, pol
...
!     write(*,*)'>>',JJ,II,K
!     write(*,'(4ES17.10)') WORK(1),WORK(2),WORK(3),WORK(4)
!     write(*,'(4ES17.10)') X(II),Y(JJ),Re(JJ,II,K),Im(JJ,II,K)
                  ENDIF
               ENDDO
               JJ=0
            ENDDO
