...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
      SUBROUTINE coefcal(nx,xvec,ny,yvec,xydata1,ldf,kx,ky,xknot,yknot,
     &     bcoef)
      implicit double precision (a-h,o-z), integer (i-n)

      parameter(nxmax=4000,kxmax=8,nymax=4000,kymax=8)
...
c
      REAL*8 xydata1(nx,ny),xydata(ldf,ldf)
      dimension work1(nxmax,nymax),work2(nxmax)
      dimension work3((2*kxmax-1)*nxmax)

      dimension xvec(nx),xknot(nx+kx),yvec(ny),yknot(ny+ky)
c      dimension xydata(ldf,*),bcoef(nx,ny)
      dimension bcoef(nx,ny)

      DO I=1, NX
         DO J=1, NY
            xydata(I,J)=xydata1(I,J)
         ENDDO
      ENDDO

      call dbs2in(nx,xvec,ny,yvec,xydata,ldf,kx,ky,xknot,yknot,bcoef)
...
cvini      dimension xydata(nx,ny),bcoef(nx,ny)
C-----VINICIUS--------------
c      DO I=1,NX
c         DO J=1, NY
c            write(2,*) xydata(I,J)
c,xydata(I,J)
c         ENDDO
c         write(2,*) 
c      ENDDO
...
c      DO I=1,NX
c         DO J=1, NY
c            write(2,*) xydata(I,J), BCOEF(I,J)
c,xydata(I,J)
c         ENDDO
c         write(2,*) 
c      ENDDO
...
c %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C     vinicius routine to use within a C routine

      SUBROUTINE SPLVALUE(x,y,kx,ky,xknot,yknot,nx,ny,bcoef,Val)
...
c      print*, "FORTRAN",x,y
      Val=dbs2vl(x,y,kx,ky,xknot,yknot,nx,ny,bcoef)
      write(1,*)x,Val
      END
...
c
      ...
      parameter(kxmax=8,kymax=8)
c     parameter(nxmax=500,nymax=500)
c-----------------vinicius 25/03/2011
      parameter(nxmax=4000,nymax=4000)
c-----------------
...
c
      ...
      parameter(nxmax=420,kxmax=8,nymax=200,kymax=8,nzmax=2050,kzmax=8)
