     ...
     &    XKNOT,YKNOT,TKNOT,nx,ny,nf,KX,nxr,nyr,FPR,NORM,PRTRMSWHOLE
     &     ,W,Z,WBCOEFRe,WBCOEFIm,nw,nz)
      IMPLICIT NONE
c------------------------------------------
c
c Purpose:
c This routine reads two sections of a wavepacket propagation into matrices,
c  given a certain range, then generates a 3D spline representation for it
c     
c
c
...
c-------------------------------------------
      INTEGER I,J,NX,NY,NF,K,IFLTH,LDF,MDF
      INTEGER II,JJ,KK,FPR(4),PRTRMSWHOLE,NOREG
      INTEGER NXR,NYR,NNX,NNY,KX
      REAL*8 X(*), Y(*), T(*), W(*),Z(*)
      REAL*8 WORK(5),NORM
...
c--- 3D SPLINE PARAMETERS
      ...
      REAL*8 Re2(NYR,NXR,NF),Im2(NYR,NXR,NF)
      REAL*8 BCOEFRe(NYR,NXR,NF),BwCOEFIm(NYR,NXR,NF)
      REAL*8 WBCOEFRe(NYR,NXR,NF),BCOEFIm(NYR,NXR,NF)
...
d     write(*,*)'>>',fpr(1),fpr(2),fpr(3),fpr(4)
         ...
         READ(10,*) wwork, wp, pol
         T(K) = T(K) * 41.3413745758D+0 ! fs -> au
         write(*,'(A20,A3,ES17.10,A5)')FILEN,',',T(K),'a.u.'
         DO I=1,NX,1
            IF(I.GE.FPR(1).AND.I.LE.FPR(2)) II = II+1
            DO J=1, NY,1
               READ(10,*) WORK(1),WORK(2),WORK(3),WORK(4)
               IF(I.GE.FPR(1).AND.I.LE.FPR(2).AND.J.GE.FPR(3).AND.
     &              J.LE.FPR(4))THEN
                  JJ = JJ +1
                  X(II)=WORK(1)
                  Y(JJ)=WORK(2)
                  Re(JJ,II,K)=WORK(3)*NORM
                  Im(JJ,II,K)=WORK(4)*NORM
               ENDIF
            ENDDO
            JJ=0
         ENDDO
         II=0
C     reactive region read as a whole
         READ(10,*) wwork, wp, pol
         DO I=1,NW,1
            DO J=1, NZ,1
               READ(10,*) W(I),Z(J),Re2(J,I,K),Im2(J,I,K)
            ENDDO
         ENDDO
...
d      WRITE(*,*)'sizes', SIZE(X),SIZE(Y),SIZE(T)
      ...
      CALL DBSNAK(NYR, Y, KX, YKNOT)
      CALL DBSNAK(NXR, X, KX, XKNOT)
...
c      write(*,*) 'real part'
      CALL DBS3IN(NYR,Y,NXR,X,NF,T,Re,NYR,NXR,
     &     KX,KX,KX,YKNOT,XKNOT,TKNOT,BCOEFRe)
c      write(*,*) 'imaginary part'
      CALL DBS3IN(NYR,Y,NXR,X,NF,T,Im,NYR,NXR,
